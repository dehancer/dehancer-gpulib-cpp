...
DHCR_KERNEL void kernel_buffer_to_texture_8b(
        DHCR_DEVICE_ARG uint8_t*     p_Input DHCR_BIND_BUFFER(0),
        texture2d_write_t      destination DHCR_BIND_TEXTURE(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
  ...
  const int index = tex.gid.y * row_bytes + tex.gid.x * 4;
  const float scale = 1.0 / 255;
  float4 color = make_float4(p_Input[index + 1] * scale, p_Input[index + 2] * scale, p_Input[index + 3] * scale, p_Input[index + 0] * scale);
  write_image(destination, color, tex.gid);
}

DHCR_KERNEL void kernel_texture_to_buffer_8b(
        texture2d_read_t          source DHCR_BIND_TEXTURE(0),
        DHCR_DEVICE_ARG uint8_t*  p_Output DHCR_BIND_BUFFER(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
...
)
{
  ...
  Texel2d tex; get_kernel_texel2d(source, tex);
  ...
  float4 color     = read_image(source, tex.gid);
  
  p_Output[index + 0] = (uint8_t)(color.w * scale);
  p_Output[index + 1] = (uint8_t)(color.x * scale);
  p_Output[index + 2] = (uint8_t)(color.y * scale);
  p_Output[index + 3] = (uint8_t)(color.z * scale);
}

DHCR_KERNEL void kernel_buffer_to_texture_16b(
        DHCR_DEVICE_ARG uint16_t*     p_Input DHCR_BIND_BUFFER(0),
        texture2d_write_t      destination DHCR_BIND_TEXTURE(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
  ...
  const int index = tex.gid.y * row_bytes / 2 + tex.gid.x * 4;
  const float scale = 1.0 / 65535;
  float4 color = make_float4(p_Input[index + 1] * scale, p_Input[index + 2] * scale, p_Input[index + 3] * scale, p_Input[index + 0] * scale);
  write_image(destination, color, tex.gid);
}

DHCR_KERNEL void kernel_texture_to_buffer_16b(
        texture2d_read_t          source DHCR_BIND_TEXTURE(0),
        DHCR_DEVICE_ARG uint16_t*  p_Output DHCR_BIND_BUFFER(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
...
)
{
  ...
  const int index = tex.gid.y * row_bytes / 2 + tex.gid.x * 4;
  const uint32_t scale = 65535;
  
  float4 color     = read_image(source, tex.gid);
  
  p_Output[index + 0] = (uint16_t)(color.w * scale);
  p_Output[index + 1] = (uint16_t)(color.x * scale);
  p_Output[index + 2] = (uint16_t)(color.y * scale);
  p_Output[index + 3] = (uint16_t)(color.z * scale);
}

DHCR_KERNEL void kernel_buffer_to_texture_32b(
        DHCR_DEVICE_ARG float*     p_Input DHCR_BIND_BUFFER(0),
        texture2d_write_t      destination DHCR_BIND_TEXTURE(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
  ...
  const int index = tex.gid.y * row_bytes / 4 + tex.gid.x * 4;
  float4 color = make_float4(p_Input[index + 1], p_Input[index + 2], p_Input[index + 3], p_Input[index + 0]);
  write_image(destination, color, tex.gid);
}

DHCR_KERNEL void kernel_texture_to_buffer_32b(
        texture2d_read_t          source DHCR_BIND_TEXTURE(0),
        DHCR_DEVICE_ARG float*  p_Output DHCR_BIND_BUFFER(1),
        DHCR_CONST_ARG uint32_t   row_bytes DHCR_BIND_BUFFER(2)
...
)
{
  ...
  p_Output[index + 0] = color.w;
  p_Output[index + 1] = color.x;
  p_Output[index + 2] = color.y;
  p_Output[index + 3] = color.z;
}

...
auto function_test =  [] (int dev_num,
    ...
    try {
      ...
      kernel.set_options({
        ...
        .luma_type = dehancer::WaveformImage::LumaType::YCbCr
      });
      kernel.set_source(input_text.get_texture());
      kernel.process();
      
      const auto& waveform = kernel.get_waveform();
  
      using ch = dehancer::math::Channel::Index;
      float clipping_low = 0.1f/100.0f;
      float clipping_high = 0.1f/100.0f;
  
      auto r = waveform.get_channel(ch::red);
      auto g = waveform.get_channel(ch::green);
      auto b = waveform.get_channel(ch::blue);
